...
export interface CareerMatchDraft {
  title: string;
  field: (typeof DOMAIN_FIELDS)[number];
  matchScore: number;
  description: string;
  avgSalary: string;
  growthRate: string;
  keySkills: string[];
  requiresHigh: Array<'technical' | 'communication' | 'analytical' | 'creativity'>;
}

const careerDatabase: CareerMatchDraft[] = [
  {
...
type DomainInterestMap = Record<(typeof DOMAIN_FIELDS)[number], number>;
...
export const generateCareerMatches = (answers: QuizAnswers) => {
  ...
  const topCareers = careerDatabase
    .map((career) => {
      ...
      if (
        parsed.technicalSkill >= careerRequires(career, 'technical') &&
...
const careerRequires = (career: CareerMatchDraft, category: 'technical' | 'communication' | 'analytical' | 'creativity') => {
...
export const buildRoadmapFromCareer = (career: CareerMatchDraft) => {
  const milestones = [
    {
